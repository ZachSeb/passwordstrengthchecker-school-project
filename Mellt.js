...
let Mellt = function() {
	...
	/**
	 * @let string CommonPasswords A letiable containing an array of common 
...
Mellt.prototype = {
	...
	CheckPassword: function(password) {
		...
		if (this.CommonPasswords) {
			...
			let text = password.toLowerCase();
			for (let t=0; t<this.CommonPasswords.length; t++) {
		...
		for (let t=0; t<password.length; t++) {
			...
			for (let characterSetKey=0; characterSetKey<this.CharacterSets.length; characterSetKey++) {
				let characterSet = this.CharacterSets[characterSetKey];
		...
		let attempts = 0;
		let charactersInBase = base.length;
		let charactersInPassword = password.length;
		for (let position=0; position<charactersInPassword; position++) {
			...
			let powerOf = charactersInPassword - position - 1;
			// Character position within the base set. We add one on because strpos is base 
			// 0, we want base 1.
			let charAtPosition = base.indexOf(password[position])+1;
			...
			// 2,3...15,16,17...23,24,25 (got it).
				...
				let multiplier = Math.pow(charactersInBase,powerOf);
				// New attempts is the number of attempts we're adding for this position.
				let newAttempts = charAtPosition * multiplier;
		...
		let perDay = this.HashesPerSecond*60*60*24;
